# 🩺 Seha-Quest
SehaQuest is a gamified preventive healthcare platform that helps users build sustainable health habits by transforming real-life activities into an interactive digital experience. Through a living digital health twin, users can visualize how daily choices impact their future health—without medical jargon, charts, or guilt-based reminders.

## 📌 Problem Statement

In modern cities like Dubai and Sharjah, people are aware of what a healthy lifestyle looks like.
However, consistency remains the biggest challenge.

Despite thousands of health and fitness applications:

- Physical inactivity is increasing
- Stress levels are rising
- Lifestyle-related diseases continue to grow

The problem is not awareness.
The problem is behavior change.

## 💡 Solution

SehaQuest addresses this challenge by turning preventive healthcare into a game.

Instead of reminders and dashboards, users interact with:

- A digital health twin that reflects their real-life habits
- A city-based virtual world shaped by daily activity
- Gamified challenges that reward healthy behavior naturally

Every step, walk, or active choice in real life directly improves the user’s in-game health and future outcomes.

## 🎮 Core Features
🧍 Digital Health Twin

- A virtual representation of the user
- Evolves based on activity, nutrition, hydration, and rest
- Shows long-term health impact visually (5–10 year outlook)

### 🌆 City-Based Gameplay

- Explore a virtual city inspired by real urban environments
- Daily habits shape the growth and condition of the city
- Unlock new zones and quests through healthy actions

### 📊 Real-World Data Integration

Automatic activity syncing with:

- Apple Health
- Samsung Health

Real steps = real progress

No manual tracking required

### 🧠 AI-Driven Insights

- Personalized health recommendations
- No medical terminology or complex charts
- Focus on motivation, not pressure

## 👥 Target Audience

- Youth and students
- Working professionals
- Universities and corporate wellness programs

## 📈 Market Opportunity

Over 3 billion mobile gamers globally

Strong shift from reactive healthcare to preventive wellness

Ideal launch environment:

- 90%+ smartphone penetration
- Alignment with Smart Living and Vision-based city initiatives

### 🆚 Competitive Advantage

While apps like Finch, Zombies, Run!, and Walkr gamify wellness, they focus on single activities.

SehaQuest stands out by:

- Offering a living digital health twin, not just tracking
- Visualizing future health outcomes
- Using game-based motivation instead of reminders
- Making health progress visible and engaging

## 💰 Monetization

Premium Plan: AED 79 / year

Premium Features:
- Future health previews
- Advanced AI insights
- Exclusive city quests
- Customization options

Revenue Projection:
- 50,000 downloads
- 6% conversion rate

Estimated annual revenue: AED 370,000 – 400,000

## 🛠️ Technology Stack

- Mobile Development: Flutter, Android Studio
- Health Data: Apple Health, Samsung Health
- Backend & Storage: Firebase / MySQL
- AI & Personalization: Python-based models

## 🔮 Roadmap

- Student pilot programs
- City-wide wellness challenges
- Institutional partnerships
- Expanded AI health forecasting
- Community and leaderboard features

## 🌱 Vision

SehaQuest reimagines healthcare as something people want to engage with, not something they are reminded to do.

We are not asking users to be disciplined.
We are making health something they want to play.

## 🧪 Figma Prototype
[![View Prototype](https://img.shields.io/badge/View-Prototype-purple)](https://www.figma.com/design/9r4M6zvhJLjIEjZtbbGGlZ/health-twin-city?node-id=0-1&t=9eJ195rB0TAwfFtM-1)

## 🎥 Watch the Project Video
[Watch the video](https://vimeo.com/1151306685?share=copy&fl=sv&fe=ci)

## 📜 License
This project is licensed for **educational and research purposes only**.  
Commercial use is **not permitted** without prior authorization.
